...
# Define subject number (Use BIDS number not actual number)
...
anat_path = f"/neurospin/unicog/protocols/IRMf/LPP_7T_Lecture_Pallier_2024/data/BidsDataset/sub-{SUBJECT_NUMBER:02}/anat"
uni_path = f"{anat_path}/sub-{SUBJECT_NUMBER:02}_uni.nii.gz"
inv1_path = f"{anat_path}/sub-{SUBJECT_NUMBER:02}_inv1.nii.gz"
inv2_path = f"{anat_path}/sub-{SUBJECT_NUMBER:02}_inv2.nii.gz"
uniden_output_path = f"{anat_path}/sub-{SUBJECT_NUMBER:02}_T1w.nii.gz"

multiplyingFactor = 10

print('using multiplying factor of {}'.format(multiplyingFactor))

mp2rage_genUniDen(uni_path, inv1_path,
inv2_path, uniden_output_path, multiplyingFactor)
